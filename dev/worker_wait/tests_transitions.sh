...
test5_open_handle() {
    ...
    create_worker w1 "$PROJ5" "$SID5" working 0 1 >/dev/null
    ...
    ( cd "$PROJ5" && exec bash "$BIN" wait --timeout "$WAIT_CEILING" > "$OUT5_FILE" 2>&1 ) &
    P5=$!
    require_trace "test5" "pid=$P5 .*status=working"
    set_hook_status "$SID5" idle "$PROJ5"
    require_trace "test5" "pid=$P5 .*status=idle bg=yes" 4
    ...
    OUT5=$(cat "$OUT5_FILE")
    ELAPSED5=$((T1 - T0))
    if [ "$OUT5" = "workers idle" ] && [ "$RC5" = 0 ] && [ "$ELAPSED5" -le "$HANG_GUARD" ]; then
...
test7_no_hook_self_heals() {
    ...
    T0=$(date +%s)
    OUT7_FILE="/tmp/${TEST_TAG}-7.out"
    ( cd "$PROJ7" && exec bash "$BIN" wait --timeout "$WAIT_CEILING" > "$OUT7_FILE" 2>&1 ) &
    P7=$!
    require_trace "test7" "pid=$P7 .*status=working"
    go_quiet "$PROJ7"
    wait "$P7"
    OUT7=$(cat "$OUT7_FILE")
    T1=$(date +%s)
    ELAPSED7=$((T1 - T0))
    if [ "$OUT7" = "workers idle" ] && [ "$ELAPSED7" -le $((WAIT_CEILING)) ]; then
        pass "test7a no-hook-entry-self-heals: reason='$OUT7' elapsed=${ELAPSED7}s"
    else
        fail "test7a no-hook-entry-self-heals: reason='$OUT7' elapsed=${ELAPSED7}s (expected 'workers idle')"
    fi
    rm -f "$OUT7_FILE"
...
test8_stuck_dead_from_start() {
    ...
    T1=$(date +%s)
    ELAPSED8=$((T1 - T0))
    if [ "$OUT8" = "timeout" ] && [ "$ELAPSED8" -ge 25 ] && [ "$ELAPSED8" -le $((25 + HANG_GUARD)) ]; then
        pass "test8 stuck-dead-from-start: reason='$OUT8' elapsed=${ELAPSED8}s"
    else
        fail "test8 stuck-dead-from-start: reason='$OUT8' elapsed=${ELAPSED8}s (expected 'timeout', >=25s and <=$((25 + HANG_GUARD))s)"
    fi
...
test8b_working_then_child_killed() {
    ...
    ( cd "$PROJ8B" && exec bash "$BIN" wait --timeout "$WAIT_CEILING" > "$OUT8B_FILE" 2>&1 ) &
    P8B=$!
    require_trace "test8b" "pid=$P8B .*status=working"
    T0=$(date +%s)
    ...
    ELAPSED8B=$((T1 - T0))
    if [ "$OUT8B" = "worker dead" ] && [ "$RC8B" = 0 ] && [ "$ELAPSED8B" -le "$HANG_GUARD" ]; then
        pass "test8b working-then-child-killed: reason='$OUT8B' ${ELAPSED8B}s after the edge"
    else
        fail "test8b working-then-child-killed: rc=$RC8B reason='$OUT8B' elapsed=${ELAPSED8B}s (expected 'worker dead', <=${HANG_GUARD}s after edge)"
    fi
    rm -f "$OUT8B_FILE"
...
test9_mixed_dead_and_working() {
    ...
    create_worker wB "$PROJ9" "$SID9B" working 0 1 >/dev/null
    OUT9_FILE="/tmp/${TEST_TAG}-9.out"
    ( cd "$PROJ9" && exec bash "$BIN" wait --timeout "$WAIT_CEILING" > "$OUT9_FILE" 2>&1 ) &
    P9=$!
    require_trace "test9" "pid=$P9 .*status=working" 2
    if kill -0 "$P9" 2>/dev/null; then
        pass "test9a mixed-still-blocks: wait process still alive after 2 working polls (dead worker did not short-circuit the still-busy worker)"
    else
    ...
    OUT9=$(cat "$OUT9_FILE")
    ELAPSED9=$((T1 - T0))
    if [ "$OUT9" = "worker dead" ] && [ "$RC9" = 0 ] && [ "$ELAPSED9" -le "$HANG_GUARD" ]; then
...
test10_dead_with_open_bg_handle() {
    ...
    create_worker w1 "$PROJ10" "$SID10" working 0 1 >/dev/null
    ...
    ( cd "$PROJ10" && exec bash "$BIN" wait --timeout "$WAIT_CEILING" > "$OUT10_FILE" 2>&1 ) &
    P10=$!
    require_trace "test10" "pid=$P10 .*status=working"
    ...
    ELAPSED10=$((T1 - T0))
    if [ "$OUT10" = "worker dead" ] && [ "$RC10" = 0 ] && [ "$ELAPSED10" -le "$HANG_GUARD" ]; then
        ...
        fail "test10 working-then-dead-bg-open: rc=$RC10 reason='$OUT10' elapsed=${ELAPSED10}s (expected 'worker dead', <=${HANG_GUARD}s after edge)"
    fi
...
test11_second_transition_exits() {
    ...
    ( cd "$PROJ11" && exec bash "$BIN" wait --timeout "$WAIT_CEILING" > "$OUT11_FILE" 2>&1 ) &
    P11=$!
    require_trace "test11" "pid=$P11 .*status=idle bg=no" 5
    if kill -0 "$P11" 2>/dev/null; then
        pass "test11a armed-while-idle: still waiting after 5 idle polls (no prior working, gate correctly holds)"
    else
    ...
    require_trace "test11" "pid=$P11 .*status=working"
    ...
    ELAPSED11=$((T1 - T0))
    if [ "$OUT11" = "workers idle" ] && [ "$RC11" = 0 ] && [ "$ELAPSED11" -le "$HANG_GUARD" ]; then
