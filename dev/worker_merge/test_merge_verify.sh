...
strand_merge_then_rerun() {
    ...
    OUT=$("$WCLI" merge feat1 "$TMPPROJ" 2>"$STRAND_DIR/err1.txt")
    RC=$?
    ...
    OUT2=$("$WCLI" merge feat1 "$TMPPROJ" 2>"$STRAND_DIR/err2.txt")
    RC2=$?
...
strand_conflict() {
    ...
    OUT3=$("$WCLI" merge conflict1 "$TMPPROJ" 2>"$STRAND_DIR/err3.txt")
    RC3=$?
